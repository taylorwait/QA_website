...
class CommentControl(View):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.error(u'[CommentControl]此话题不存在:[%s]' % en_title)
        ...
        if text.startswith('@['):
            ...
            try:
                ...
                logger.error(u'[CommentControl]回答引用错误:%s' % parent_str)
                return HttpResponse(u"请勿修改回答代码！", status=403)

        if not text:
            logger.error(
                u'[CommentControl]当前用户输入空回答:[{}]'.format(
                    user.username
                )
            )
            return HttpResponse(u"请输入回答内容！", status=403)
        ...
        print_comment = u"<p>回答：{}</p>".format(text)
        if parent:
