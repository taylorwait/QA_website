...
# 功能
1. 话题,分类,专栏的添加，删除，修改。支持**tinymce**富文本编辑器。支持话题中代码**高亮**。
...
4. **支持回答**，实现了一个独立的回答系统。
5. 首页支持显示**轮播**，显示最新回答，显示人气最高的话题。
6. 首页支持显示**标签云**，拥有一个非常酷炫的便签云。
7. 拥有一个**动态加载**的“全部话题”板块 可以显示所有话题分类，可以按照浏览数或者时间排序显示话题。
...
![回答](http://vmaig.qiniudn.com/screenshot/vmaig_03.jpg)
...
设置其中的  PAGE\_NUM 每页显示话题数，EMAIL\_HOST(你用的邮箱的smtp)，EMAIL\_PORT(smtp端口)，EMAIL\_HOST\_USER(你的邮箱的用户名)，EMAIL\_HOST\_PASSWORD(你的邮箱密码)，如果要使用七牛设置好七牛的相关配置。  
